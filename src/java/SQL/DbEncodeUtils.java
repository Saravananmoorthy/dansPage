...
public class DbEncodeUtils {
    ...
    /**
     * convert encode val (a date) into the (Integer)-th position of Prepared
     * statement. mostly checking if null, need to encode that specially.
     */
    public static String encodeDate(PreparedStatement ps, int position, java.sql.Date val) {
        ...
        if (val == null) {
            try {
                ...
                msg = "**** EncodeForDbUtils.encodeDate():  exception trying "
                        + "to encode null (Date) into position number "
            ...
            try {
                ...
                msg = "**** EncodeForDbUtils.encodeDate(): exception trying to "
                        + "encode (Date) value [" + val.toString()
    ...
    public static String encodeInteger(PreparedStatement ps, int position, Integer val) {
        ...
        if (val == null) {
            try {
                ...
                msg = "**** EncodeForDbUtils.encodeInteger(): exception trying"
                        + " to encode null (Integer) into position number "
                        + position + " of prepared sql statement. Error: "
                        + e.getMessage();
            }
        } else {
            try {
                ...
                msg = "**** EncodeForDbUtils.encodeInteger(): exception "
                        + "trying to encode value [" + val
    ...
    public static String encodeDecimal(PreparedStatement ps, int position, BigDecimal val) {
        ...
        if (val == null) {
            try {
                ...
                msg = "**** EncodeForDbUtils.encodeDecimal(): exception trying "
                        + "to encode null (Decimal) into position number "
                        + position + " of prepared sql statement. Error: "
                        + e.getMessage();
            }
        } else {
